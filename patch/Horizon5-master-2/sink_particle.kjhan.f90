...
subroutine kjhan_make_sink(ilevel)
  ...
  integer::n_before_prox,n_blocked_prox,n_before_all,n_blocked_all
  integer::n_leaf,n_pass_rhostar,n_pass_dstar,n_pass_djeans,n_pass_quench
  integer::n_leaf_all,n_pass_rhostar_all,n_pass_dstar_all,n_pass_djeans_all,n_pass_quench_all
...
!$omp parallel do private(igrid,ngrid,iskip,ind,i,d,u,v,w,e,bx1,by1,bz1,bx2,by2,bz2) 
  ...
  ntot=0
  ! DEBUG: diagnostic counters
  n_before_prox=0; n_blocked_prox=0
  n_leaf=0; n_pass_rhostar=0; n_pass_dstar=0; n_pass_djeans=0; n_pass_quench=0
  if(myid==1) write(*,'(A,I8,A,I8)') &
     ' [DEBUG make_sink] level=',ilevel,' nsink=',nsink
  ! Loop over grids
  ncache=active(ilevel)%ngrid
!$omp parallel do private(igrid,ngrid,i,ind,iskip, d, star_ratio, temp, d_jeans, d_thres, x,y,z,isink, dxx, dr_sink, dyy, dzz ) reduction(+: ntot, n_before_prox, n_blocked_prox, n_leaf, n_pass_rhostar, n_pass_dstar, n_pass_djeans, n_pass_quench) schedule(static)
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
     do i=1,ngrid
       ind_grid(i)=active(ilevel)%igrid(igrid+i-1)
...
!!$      if(star.and.star_ratio<star_ratio_floor)ok(i)=.false.
          ...
          ! DEBUG: count after d_star check
          if(ok(i)) n_pass_dstar=n_pass_dstar+1
          ! Check if gas is Jeans unstable
          if(d      <d_thres)ok(i)=.false.
          ! DEBUG: count after d_jeans check
          if(ok(i)) n_pass_djeans=n_pass_djeans+1
          ! Quenching criterion
           if(flag2(ind_cell(i))==1)ok(i)=.false.
          ! DEBUG: count after quenching check
          if(ok(i)) n_pass_quench=n_pass_quench+1

          if(ok(i))then
             n_before_prox=n_before_prox+1
             ...
             enddo
             if(.not.ok(i)) n_blocked_prox=n_blocked_prox+1
          endif
  ...
  ! DEBUG: print diagnostic counters
#ifndef WITHOUTMPI
  call MPI_ALLREDUCE(n_before_prox,n_before_all,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
  call MPI_ALLREDUCE(n_blocked_prox,n_blocked_all,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
  call MPI_ALLREDUCE(n_leaf,n_leaf_all,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
  call MPI_ALLREDUCE(n_pass_rhostar,n_pass_rhostar_all,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
  call MPI_ALLREDUCE(n_pass_dstar,n_pass_dstar_all,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
  call MPI_ALLREDUCE(n_pass_djeans,n_pass_djeans_all,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
  call MPI_ALLREDUCE(n_pass_quench,n_pass_quench_all,1,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
#else
  n_before_all=n_before_prox; n_blocked_all=n_blocked_prox
  n_leaf_all=n_leaf; n_pass_rhostar_all=n_pass_rhostar; n_pass_dstar_all=n_pass_dstar
  n_pass_djeans_all=n_pass_djeans; n_pass_quench_all=n_pass_quench
#endif
  if(myid==1) write(*,'(A,I8,A,I8,A,I8,A,I8)') &
     ' [DEBUG] lv=',ilevel,' before_prox=',n_before_all,' blocked=',n_blocked_all, &
     ' ntot=',ntot
  if(myid==1) write(*,'(A,I8,A,I8,A,I8,A,I8,A,I8)') &
     ' [DEBUG] leaf=',n_leaf_all,' rhostar=',n_pass_rhostar_all,' dstar=',n_pass_dstar_all, &
     ' djeans=',n_pass_djeans_all,' quench=',n_pass_quench_all
...
!!$omp parallel do private(isink) schedule(dynamic)
  do isink=1,nsink
     if(oksink_all(isink)==1)then
        ...
        if(myid==1.and.nstep_coarse>=89.and.nstep_coarse<=99) then
           write(*,'(A,I5,A,I4,3(A,ES20.12))') &
                'XSINK create_sink step=',nstep_coarse, &
                ' id=',idsink(isink), &
                ' x=',xsink(isink,1),' y=',xsink(isink,2),' z=',xsink(isink,3)
        endif
     endif
  end do
...
!$omp parallel do private(isink,xx,yy,zz)
  ...
  ! XSINK diagnostic: sink positions after merge_sink
  if(myid==1.and.nstep_coarse>=89.and.nstep_coarse<=99) then
     do isink=1,nsink
        write(*,'(A,I5,A,I4,3(A,ES20.12))') &
             'XSINK merge_sink step=',nstep_coarse, &
             ' id=',idsink(isink), &
             ' x=',xsink(isink,1),' y=',xsink(isink,2),' z=',xsink(isink,3)
     end do
  endif
...
!$omp parallel do private(isink,ncloud,idim,vdum)
  ...
  ! XSINK diagnostic: sink positions after grow_sink
  if(myid==1.and.nstep_coarse>=89.and.nstep_coarse<=99) then
     do isink=1,nsink
        write(*,'(A,I5,A,I4,3(A,ES20.12))') &
             'XSINK grow_sink step=',nstep_coarse, &
             ' id=',idsink(isink), &
             ' x=',xsink(isink,1),' y=',xsink(isink,2),' z=',xsink(isink,3)
     end do
  endif

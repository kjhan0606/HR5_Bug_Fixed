...
subroutine init_flow_fine(ilevel)
  ...
  integer::dum,ielt!,iHydrogen,iHelium
  character(LEN=1)::a1
  character(LEN=2)::nelt_str
  character(LEN=3),allocatable,dimension(:) ::elem_list_local
  ...
  !---------------------------------------------
  ! Get index of hydrogen and helium in gas grid
  !---------------------------------------------
  ...
  read(34,format_string)a1,elem_list_local
  close(34)
  iHydrogen=-1
  iHelium=-1
  do ielt=1,nelt
     if(elem_list_local(ielt)=='H')  iHydrogen=ichem+ielt-1

  ...
  if(sink)then
     ...
     do isink=1,nsink
        if(oksink_all(isink)==1d0)then
           ...
           xold(1:ndim)=xsink(isink,1:ndim)
           ...
           if(myid==1.and.nstep_coarse>=89.and.nstep_coarse<=99) then
              write(*,'(A,I5,A,I3,A,I4,3(A,ES20.12),A,ES12.4)') &
                   'XSINK move_fine step=',nstep_coarse,' lv=',ilevel, &
                   ' id=',idsink(isink), &
                   ' x=',xsink(isink,1),' y=',xsink(isink,2),' z=',xsink(isink,3), &
                   ' dr=',sqrt(sum((xsink(isink,1:ndim)-xold(1:ndim))**2))
           endif
        endif
     end do
  endif

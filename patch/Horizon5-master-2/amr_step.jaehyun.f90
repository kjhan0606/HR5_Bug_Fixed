...
!!$  logical::ok_defrag
  ...
  ! Diagnostic: energy checksum for divergence debug
  real(dp) :: diag_esum, diag_esum_all, diag_phisum, diag_phisum_all
  integer  :: diag_ilev, diag_igrid, diag_ind, diag_icell
  character(LEN=80) :: diag_filename
  real(dp) :: diag_x, diag_y, diag_z, diag_dx
  integer  :: diag_ix, diag_iy, diag_iz
...
!jhshin2
  ...
  if(ilevel==levelmin)then
     ...
     ! DIAG: before feedback (A0)
     if(nstep_coarse>=89.and.nstep_coarse<=92) then
        diag_esum=0d0; diag_phisum=0d0
        do diag_ilev=levelmin,nlevelmax
           do diag_igrid=1,active(diag_ilev)%ngrid
              do diag_ind=1,twotondim
                 diag_icell=active(diag_ilev)%igrid(diag_igrid)+ncoarse+(diag_ind-1)*ngridmax
                 if(son(diag_icell)==0) then
                    diag_esum=diag_esum+uold(diag_icell,ndim+2)
                    diag_phisum=diag_phisum+phi(diag_icell)
                 endif
              end do
           end do
        end do
        call MPI_ALLREDUCE(diag_esum,diag_esum_all,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
        call MPI_ALLREDUCE(diag_phisum,diag_phisum_all,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
        if(myid==1) write(*,'(A,I5,A,ES22.15,A,ES22.15)') &
             ' DIAG step=',nstep_coarse,' [A0:before_feedback] esum=',diag_esum_all,' phisum=',diag_phisum_all
     endif
     ...
     if(nstep_coarse>=89.and.nstep_coarse<=92) then
        ...
        call MPI_ALLREDUCE(diag_phisum,diag_phisum_all,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
        if(myid==1) write(*,'(A,I5,A,ES22.15,A,ES22.15)') &
             ' DIAG step=',nstep_coarse,' [A:after_feedback] esum=',diag_esum_all,' phisum=',diag_phisum_all
     endif
  ...
  ! DIAG: after create_sink (levelmin only)
  if(ilevel==levelmin.and.nstep_coarse>=89.and.nstep_coarse<=92) then
     diag_esum=0d0
     do diag_ilev=levelmin,nlevelmax
        do diag_igrid=1,active(diag_ilev)%ngrid
           do diag_ind=1,twotondim
              diag_icell=active(diag_ilev)%igrid(diag_igrid)+ncoarse+(diag_ind-1)*ngridmax
              if(son(diag_icell)==0) diag_esum=diag_esum+uold(diag_icell,ndim+2)
           end do
        end do
     end do
     call MPI_ALLREDUCE(diag_esum,diag_esum_all,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
     if(myid==1) write(*,'(A,I5,A,ES22.15)') &
          ' DIAG step=',nstep_coarse,' [B:after_create_sink] esum=',diag_esum_all
  endif
...
!############################################################
     ...
     ! DIAG: after Poisson solver — dump phi, rho, etot (leaf only)
     if(nstep_coarse==89) then
        diag_dx = 0.5d0**ilevel
        write(diag_filename,'("diag_poisson_",I5.5,"_L",I2.2,"_",I5.5,".dat")') nstep_coarse, ilevel, myid
        open(unit=669,file=trim(diag_filename),form='formatted')
        write(669,'(A)') '# x  y  z  phi  rho  etot'
        do diag_igrid=1,active(ilevel)%ngrid
           do diag_ind=1,twotondim
              diag_icell=active(ilevel)%igrid(diag_igrid)+ncoarse+(diag_ind-1)*ngridmax
              if(son(diag_icell)==0) then
                 diag_iz=(diag_ind-1)/4
                 diag_iy=(diag_ind-1-4*diag_iz)/2
                 diag_ix=(diag_ind-1-4*diag_iz-2*diag_iy)
                 diag_x=(xg(active(ilevel)%igrid(diag_igrid),1)+(dble(diag_ix)-0.5d0)*diag_dx)
                 diag_y=(xg(active(ilevel)%igrid(diag_igrid),2)+(dble(diag_iy)-0.5d0)*diag_dx)
                 diag_z=(xg(active(ilevel)%igrid(diag_igrid),3)+(dble(diag_iz)-0.5d0)*diag_dx)
                 write(669,'(3ES18.10,3ES22.14)') diag_x, diag_y, diag_z, &
                      phi(diag_icell), rho(diag_icell), uold(diag_icell,ndim+2)
              endif
           end do
        end do
        close(669)
...
!    call MPI_BARRIER(MPI_COMM_WORLD,mpi_err)
        ...
        ! DIAG: after synchro_hydro_fine — dump (leaf only)
        if(nstep_coarse==89) then
           diag_dx = 0.5d0**ilevel
           write(diag_filename,'("diag_synchro_",I5.5,"_L",I2.2,"_",I5.5,".dat")') nstep_coarse, ilevel, myid
           open(unit=669,file=trim(diag_filename),form='formatted')
           write(669,'(A)') '# x  y  z  phi  rho  etot'
           do diag_igrid=1,active(ilevel)%ngrid
              do diag_ind=1,twotondim
                 ...
                 if(son(diag_icell)==0) then
                    ...
                    diag_z=(xg(active(ilevel)%igrid(diag_igrid),3)+(dble(diag_iz)-0.5d0)*diag_dx)
                    write(669,'(3ES18.10,3ES22.14)') diag_x, diag_y, diag_z, &
                         phi(diag_icell), uold(diag_icell,1), uold(diag_icell,ndim+2)
                 endif
              end do
           end do
           close(669)
...
!############################################################
  ...
  if(hydro)then
     ...
     ! DIAG: after godunov+set_uold — dump (leaf only)
     if(nstep_coarse==89) then
        diag_dx = 0.5d0**ilevel
        write(diag_filename,'("diag_godunov_",I5.5,"_L",I2.2,"_",I5.5,".dat")') nstep_coarse, ilevel, myid
        open(unit=669,file=trim(diag_filename),form='formatted')
        write(669,'(A)') '# x  y  z  phi  rho  etot'
        do diag_igrid=1,active(ilevel)%ngrid
           do diag_ind=1,twotondim
...
!    call MPI_BARRIER(MPI_COMM_WORLD,mpi_err)
  ...
  ! DIAG: after cooling — dump (leaf only)
  if(nstep_coarse==89) then
     diag_dx = 0.5d0**ilevel
     write(diag_filename,'("diag_cooling_",I5.5,"_L",I2.2,"_",I5.5,".dat")') nstep_coarse, ilevel, myid
     open(unit=669,file=trim(diag_filename),form='formatted')
     write(669,'(A)') '# x  y  z  phi  rho  etot'
     do diag_igrid=1,active(ilevel)%ngrid
        do diag_ind=1,twotondim

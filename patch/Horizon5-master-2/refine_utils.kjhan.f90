...
subroutine refine_coarse
  ...
  ncreate=0
  increate = 0
!$omp parallel do private(i,j,k,ind) reduction(+:increate)
  do k=0,nz-1
  do j=0,ny-1
  do i=0,nx-1
     ind=1+i+j*nx+k*nxny
...
!!! This part should not be parallelized due to the atomic functions of linked
!!!!!                    list.
...
!!!!!                    list.
  ...
  if(verbose) write(*,*) 'used_mem = ', used_mem
...
!#######################################################################
!#######################################################################
subroutine sub2_refine_fine(icpu,ilevel, igrid,ngrid, ibound, boundary_region,increate)
  use amr_commons
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
#endif
  integer::ilevel,increate
  !---------------------------------------------------------
...
end subroutine sub2_refine_fine
!#######################################################################
!#######################################################################
!#######################################################################
!#######################################################################
subroutine sub1_refine_fine(icpu,ilevel, igrid,ngrid, ibound, boundary_region)
  use amr_commons
  implicit none
#ifndef WITHOUTMPI
  include 'mpif.h'
...
end subroutine sub1_refine_fine
!###############################################################
!###############################################################
!###############################################################
!###############################################################
subroutine refine_fine(ilevel)
  ...
  integer::igrid,icell,i
  integer::ind,iskip,info,icpu,ibound
  integer::ncreate_tmp,nkill_tmp,increate,jncreate
  logical::boundary_region
  integer,dimension(1:nvector)::ind_grid,ind_cell
  integer,dimension(1:nvector)::ind_grid_tmp,ind_cell_tmp
  logical,dimension(1:nvector)::ok
  ...
  do icpu=1,ncpu+nboundary  ! Loop over cpus and boundaries
     ...
     jncreate = 0
! This is not parallelized due to the atomic functions on the linked list being spread over the subroutines.
!!!$omp parallel do private(igrid,ngrid,increate) reduction(+:jncreate) 
     do igrid=1,ncache,nvector  ! Loop over grids
        ngrid=MIN(nvector,ncache-igrid+1)
        call sub2_refine_fine(icpu,ilevel, igrid,ngrid, ibound, boundary_region,increate)
        jncreate = jncreate + increate
     end do
     ncreate = ncreate + jncreate
  end do
  if(verbose)write(*,112)ncreate
  endif
...
!!! This is not working in parallel mode because something is here!
!!$omp parallel do private(igrid,ngrid)
     do igrid=1,ncache,nvector  ! Loop over grids
        ngrid=MIN(nvector,ncache-igrid+1)
        call sub1_refine_fine(icpu,ilevel, igrid,ngrid, ibound, boundary_region)
     end do
  end do
  if(verbose)write(*,113)nkill

...
!$omp parallel do private(ipart)
        do ipart=1,nsink
           if(xsink(ipart,idim)/scale+skip_loc(idim)<0.0d0) then
                xsink(ipart,idim)=xsink(ipart,idim)+(xbound(idim)-skip_loc(idim))*scale
                if(myid==1.and.nstep_coarse>=89.and.nstep_coarse<=99) &
                   write(*,'(A,I5,A,I4,A,I2,A)') &
                   'XSINK pbc_coarse step=',nstep_coarse,' id=',idsink(ipart),' dim=',idim,' wrap+'
           endif
           if(xsink(ipart,idim)/scale+skip_loc(idim)>=xbound(idim)) then
                xsink(ipart,idim)=xsink(ipart,idim)-(xbound(idim)-skip_loc(idim))*scale
                if(myid==1.and.nstep_coarse>=89.and.nstep_coarse<=99) &
                   write(*,'(A,I5,A,I4,A,I2,A)') &
                   'XSINK pbc_coarse step=',nstep_coarse,' id=',idsink(ipart),' dim=',idim,' wrap-'
...
!$omp parallel do private(ipart)
        do ipart=1,nsink
           if(xsink(ipart,idim)/scale+skip_loc(idim)<0.0d0) then
                ...
                if(myid==1.and.nstep_coarse>=89.and.nstep_coarse<=99) &
                   write(*,'(A,I5,A,I4,A,I2,A)') &
                   'XSINK pbc_fine step=',nstep_coarse,' id=',idsink(ipart),' dim=',idim,' wrap+'
           endif
           if(xsink(ipart,idim)/scale+skip_loc(idim)>=xbound(idim)) then
                xsink(ipart,idim)=xsink(ipart,idim)-(xbound(idim)-skip_loc(idim))*scale
                if(myid==1.and.nstep_coarse>=89.and.nstep_coarse<=99) &
...
subroutine virtual_tree_fine(ilevel)
  ...
  ! ksection variables removed for SRC build
#endif

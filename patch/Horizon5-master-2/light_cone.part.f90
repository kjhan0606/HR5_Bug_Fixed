...
!jhshin1
  ...
  real(dp),dimension(:,:),allocatable::etc_out
!jhshin2    
  real(kind=8),dimension(:,:),allocatable::posout,velout
  real(kind=8),dimension(:),allocatable::zout
  real(kind=8),dimension(:,:),allocatable::tmparr
  real(dp),dimension(:,:),allocatable::xp_out,vp_out
  real(dp),dimension(:),allocatable::zp_out
...
!jaehyun1
  ...
  real(dp)::nfine,daexp,dist1,dist2,dist10,dist20,coord_distance,z10,z20,Omega0,OmegaL,OmegaR,coverH0
  real(dp)::scale_l,scale_d,scale_t,scale_v,scale_nH,scale_T2
!jaehyun2
  ...
  if(myid==1 .and. obs==1 .and. .not. hydro)  write(*,*)'lightcone redshifts',z10,z20
  if(myid==1 .and. obs==1 .and. .not. hydro)  write(*,*)'Distance (Mpc)',dist10,dist20

  ilun=3*ncpu+myid+10
  
  ! Determine the filename, dir, etc
  if(myid==1 .and. obs==1)write(*,*)'Computing and dumping lightcone (All kind of particles)'
           ...
           do jpart=1,npart1
              ...
              if(ip==nvector)then
				 ...
				 do i=1,ip
                    ...
                    if(star .or. sink) etc(2,i)=tp(ind_part(i))
                    if(star .and. metal) etc(3,i)=zp(ind_part(i))
				 end do
                !jhshin2
...
!     write(ilun,*) 1./(1.+z1)
  ...
  if(IOGROUPSIZECONE>0) then
   ...
   if (myid == 1 .and. .not. hydro) then
     ...
     rewind(ilun)
     write(ilun,'("a-da          =",E23.15)') aexp_old2-daexp
     write(ilun,'("a             =",E23.15)') aexp+daexp
     write(ilun,'("Dist(a-da)    =",E23.15)') dist2/Lbox
     write(ilun,'("Dist(a)       =",E23.15)') dist1/Lbox
     write(ilun,'("a0-da0        =",E23.15)') aexp_old2
     write(ilun,'("a0            =",E23.15)') aexp
     write(ilun,'("Dist(a0-da0)  =",E23.15)') dist20/Lbox
     write(ilun,'("Dist(a0)      =",E23.15)') dist10/Lbox
     write(ilun,'("h0            =",E23.15)') hubin
     ...
     write(ilun,'("Lbox (cMpc/h) =",E23.15)') boxlen_ini
     close(ilun)
   endif
